# Wideband SDR receiver compatible with HPSDR (Protocol 2)

## Introduction

This is the wideband variant of the [openHPSDR Protocol 2 receiver](/sdr-receiver-hpsdr2/) for the [TRX-duo](https://www.wimo.com/en/trx-duo). It trades receiver count for bandwidth, presenting fewer DDCs at higher I/Q sample rates, which is useful for panadapter/wideband monitoring with programs that support the openHPSDR Ethernet Protocol 2.
...
The FPGA configuration consists of up to **six** identical digital down-converters (DDC). Their structure is shown in the following diagram:

![Wideband HPSDR2 receiver](/img/sdr-receiver-hpsdr2-wide-ddc.png)
...
The resulting I/Q data rate is configurable, with settings of **48, 96, 192, 384, 768 kSPS** (work in progress). The tunable frequency range covers from 0 Hz to 62.5 MHz.
...
The [projects/sdr_receiver_hpsdr2_wide_trx_duo](https://github.com/fventuri/red-pitaya-notes/tree/hpsdr2/projects/sdr_receiver_hpsdr2_wide_trx_duo) directory contains the Tcl files ([block_design.tcl](https://github.com/fventuri/red-pitaya-notes/blob/hpsdr2/projects/sdr_receiver_hpsdr2_wide_trx_duo/block_design.tcl), [rx.tcl](https://github.com/fventuri/red-pitaya-notes/blob/hpsdr2/projects/sdr_receiver_hpsdr2_wide_trx_duo/rx.tcl)) that instantiate, configure and interconnect all the needed IP cores.

The [server](https://github.com/fventuri/red-pitaya-notes/tree/hpsdr2/projects/sdr_receiver_hpsdr2_wide_trx_duo/server) directory contains the source code of the UDP server that receives control commands and transmits the I/Q data streams to the SDR programs.

## Wideband display

In addition to the six narrow-band DDC receivers, this version implements the openHPSDR Protocol 2 **wideband display** for **ADC0** — the full 0 Hz – 62.5 MHz panorama shown in the "wideband" panadapter / waterfall window of the SDR program.
...
- Optionally, to start the application automatically at boot time, copy its `start.sh` file from `apps/sdr_receiver_hpsdr2_wide_trx_duo` to the topmost directory on the SD card.
...
Building `sdr_receiver_hpsdr2_wide_trx_duo.bit`:

```bash
make NAME=sdr_receiver_hpsdr2_wide_trx_duo bit
```
