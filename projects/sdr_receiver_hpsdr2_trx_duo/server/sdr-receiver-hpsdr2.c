...
#define FIFO_WORD          64      /* bytes per instant in the DDR ring: 16 * 4B LE
                                      channels (DDC0_I, DDC0_Q, DDC1_I ... DDC7_Q),
                                      each a 26-bit signed value in a 32-bit LE word */
#define RATE_BASE          48000   /* rx_rate = RATE_BASE/rate_khz; 48000 for the
                                      2/5 FIR chain (48-192): 48->1000, 96->500, 192->250 */
...
/* The 2/5 FIR chain already sits at the correct ~-105 dBFS floor (no axis_gain — that
   was only needed for the wide project's 4/5 chain), so build_packet is just the
   24-bit big-endian gather. */
...
static inline void build_packet(uint8_t *pkt, const uint8_t *raw, int ch)
{
  ...
  /* Each raw instant is 16 * 4B LE channels; DDC ch has I at sp[0..3], Q at sp[4..7]
...
void *reader_thread(void *arg)
{
  int ch;
  (void)arg;
  const uint32_t nbytes = SAMPLES_PER_FRAME * FIFO_WORD;   /* one packet: 238 * 64 */
  while(1)
  {
...
int main(int argc, char *argv[])
{
  ...
  *rx_rst &= ~1; *rx_rst |= 1;
  ...
  /* Keep the reader (DDR-ring poll + copy) on core 0 and put the sender, status
     thread and this command/main thread together on core 1. 8 DDC @ <=192 ksps is
