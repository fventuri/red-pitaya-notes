...
if grep -q '
' $apps_dir/sdr_transceiver_ft8_125M/upload-ft8.sh
then
  mount -o rw,remount /media/mmcblk0p1
  dos2unix $apps_dir/sdr_transceiver_ft8_125M/upload-ft8.sh
  mount -o ro,remount /media/mmcblk0p1
fi

rm -rf /dev/shm/*

cat $apps_dir/sdr_transceiver_ft8_125M/sdr_transceiver_ft8_125M.bit > /dev/xdevcfg

ln -sf $apps_dir/sdr_transceiver_ft8_125M/ft8.cron /etc/cron.d/ft8
